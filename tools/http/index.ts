...
import { genConfig, transformConfigByMethod } from "./config";
...
class EnclosureHttp {
  ...
  private static genUniqueKey(config: EnclosureHttpRequestConfig): string {
    let _data = config.data
     if(/GET/i.test(config.method+'')) _data = config.params;
    //  console.log(`${config.url}--${JSON.stringify(_data)}`);
    
    return `${config.url + ''+ Math.round(Math.random()*100)}--${JSON.stringify(_data)}`;
  }
