  ...
  const   navRefs = ref<any>(null)
    ...
    return () => {
      ...
      const items = (slots.default?.() || [])
        .filter((vnode) => (vnode.type as Component).name === 'DemoGroupItem')
        .map((vnode) => {
          if (vnode.props === null) {
            vnode.props = {}
          }
      ...
      return h('div', { class: 'demo-group' }, [
     
        h(
          'div',
          { class: 'demo-group__nav' ,ref: (element:HTMLDivElement) => {
            if (element) {
               (!navRefs.value||navRefs.value>element.clientWidth)&&(navRefs.value = element.clientWidth)
            }
          }, },
          h(
            'ul',
            { class: 'demo-group__ul' },
            items.map((vnode, i) => {
              const isActive = i === activeIndex.value

              return h(
                'li',
                { class: 'demo-group__li' },
                h(
                  'button',
                  {
                    ref: (element) => {
                    ...
                    class: {
                      ...
                      'demo-group__nav-tab-active': isActive,
                    },
        ...
        h(
          'div',
          { class: 'demo-group__content', style: {width:  'calc(100% - '+ (navRefs.value-5)+'px)' } },
          items,
        )
      ])
    }
